...
@implementation SCAVENGERCJSONDeserializer (CJSONDeserializer_BlocksExtensions)
...
- (void)deserializeAsDictionary:(NSData *)inData completionBlock:(void (^)(id result, NSError *error))block {
	...
	if (inData == NULL || [inData length] == 0) {
		noDataError = [NSError errorWithDomain:SCAVENGERkJSONDeserializerErrorDomain code:kJSONScannerErrorCode_NothingToScan userInfo:NULL];
...
- (void)deserializeAsArray:(NSData *)inData completionBlock:(void (^)(id result, NSError *error))block {
	...
	if (inData == NULL || [inData length] == 0) {
		nullInDataError = [NSError errorWithDomain:SCAVENGERkJSONDeserializerErrorDomain code:kJSONScannerErrorCode_NothingToScan userInfo:NULL];
